...
def gen_number(n_int=10, n_frac=0, positive=True, cnt=1):
    ...
    for i in range(0,cnt):
        ...
        if (n_frac > 0):
            number += 10**(-1*n_frac) * randint(10**(n_frac-1), 10**n_frac-1)
        if (not positive):
            number = -1*number
        recs[i] = number    
...
def gen_string(n=10, category='alpha', cnt=1):
    ...
    if (category not in ['alpha', 'lower', 'upper', 'numeric', 'alphanumeric', 'special']):
        raise ValueError('category must be alpha|lower|upper|numeric|alphanumeric|special')
...
def gen_date(date_format='iso', start=None, end=None, current=None, time_zone=None, cnt=1):
    ...
    if (start != None or end != None):
        start = datetime.strptime(start, date_format) if (start != None) else datetime.now(time_zone)
        end = datetime.strptime(end, date_format) if (end != None) else datetime.now(time_zone)           
        if (end < start):
            raise ValueError('Invalid interval end < start') 
        if (version_info[0] == 2 and version_info[1] == 6):
            td = end-start
            delta = int((td.microseconds + (td.seconds + td.days * 24 * 3600) * 10**6) / 10**6)
        else:                   
                        ...
                        if (current != 'day'):
                            ...
                            if (current != 'month'):                            
                                if (day < 29):
                                    ...
                                    month = [1, 3, 5, 7, 8, 10, 12][randint(0,6)]
             
                recs[i] = datetime(year, month, day, hour, minute, second, tzinfo=time_zone)
...
def gen_birth_no(min_age=18, max_age=30, male=True, delimiter=False, cnt=1):    
    ...
    recs = [None]*cnt
    for i in range(0,cnt):
        dt = datetime.strftime(start + timedelta(days=randint(0,delta)), '%Y%m%d')    
...
def gen_reg_no(cnt=1):
    ...
    recs = [None]*cnt
    for i in range(0,cnt):
        n = [randint(0,9), randint(0,9), randint(0,9), randint(0,9), randint(0,9), randint(0,9), randint(0,9), 0]
        rem = (n[0]*8 + n[1]*7 + n[2]*6 + n[3]*5 + n[4]*4 + n[5]*3 + n[6]*2) % 11
        n[7] = (11-rem) % 10
...
def gen_account_no(form='nat', country='CZ', prefix=False, bank=None, base_len=10, prefix_len=6, cnt=1):    
    ...
    if (bank == None):
        ...
        query = 'SELECT code FROM bank LIMIT {0} OFFSET ABS(RANDOM()) % (SELECT count(*) FROM bank)'.format(cnt if (cnt < 100) else 100)
    ...
    for i in range(0,cnt):
        ...
        for j in range(10-base_len,9):
            base[j] = randint(0,9)
        rem = (base[0]*6 + base[1]*3 + base[2]*7 + base[3]*9 + base[4]*10 + base[5]*5 + base[6]*8 + base[7]*4 + base[8]*2) % 11
        ...
        if (prefix):
            ...
            for j in range(6-prefix_len,5):
                pref[j] = randint(0,9)
            rem = (pref[0]*10 + pref[1]*5 + pref[2]*8 + pref[3]*4 + pref[4]*2) % 11
            pref[5] = (11-rem) % 10 if (rem > 0) else 0
        
        bank_code = bank if (bank != None) else str(banks[randint(0,bank_len-1)][0])    
        if (form == 'nat'):
            recs[i] = ''.join(map(str,base[10-base_len:])) + '/' + bank_code
            if (prefix):
                recs[i] = ''.join(map(str,pref[6-prefix_len:])) + '-' + recs[i]
        else:
...
def gen_email(name_len=8, subdomain=None, subdomain_len=6, domain='.com', domain_type='original', cnt=1):
    ...
    subdomains = [subdomain]*cnt if (subdomain != None) else gen_string(subdomain_len, 'lower', cnt)
    if (cnt == 1):
        names = [names]
        if (subdomain == None):
    ...
    for i in range(0,cnt):
        ...
        email += domain if (domain != None) else str(domains[randint(0,domain_len-1)][0])
        recs[i] = email
...
def gen_name(sex='both', tuple_out=True, cnt=1):
    ...
    db = DBO(_get_dsn())._dbo_driver
    if (sex == 'both'):
        query = 'SELECT name, sex FROM first_name  LIMIT {0} OFFSET ABS(RANDOM()) % (SELECT count(*) FROM first_name)'.format(cnt if (cnt < 100) else 100)
        ...
        filter = 'WHERE sex = \'M\'' if (sex == 'male') else 'WHERE sex = \'F\''
    ...
    surname_len = len(surnames) if (sex != 'both') else [len(surnames[0]), len(surnames[1])]    
    recs = [None]*cnt
    for i in range(0,cnt):
        if (sex == 'both'):
            first_name, db_sex = first_names[randint(0,first_name_len-1)]
            surname = surnames[0][randint(0,surname_len[0]-1)][0] if (db_sex == u'M') else surnames[1][randint(0,surname_len[1]-1)][0]
        else:
        ...
        if (version_info[0] == 2):
            ...
            recs[i] = (first_name.encode('utf-8').decode(), surname.encode('utf-8').decode())      
...
def gen_phone(form='int', cc=420, country=None, ndc=601, sn_len=6, cnt=1):
    ...
    if (cc == None):
        db = DBO(_get_dsn())._dbo_driver
        query = 'SELECT code FROM cc LIMIT {0} OFFSET ABS(RANDOM()) % (SELECT count(*) FROM cc)'.format(cnt if (cnt < 100) else 100)
        codes = db.execute(query).fetchall()
        codes_len = len(codes)
    elif (country != None):
        db = DBO(_get_dsn())._dbo_driver
        query = 'SELECT code FROM cc WHERE title = \'{0}\' LIMIT 1'.format(country)
    ...
    for i in range(0,cnt):
        if (form == 'int'):
            phone = '+' + str(cc) if (cc != None) else '+' + str(codes[randint(0,codes_len-1)][0])
        else:
            phone = ''
...
def gen_address(param=None, value=None, street_no_full=True, dict_out=True, cnt=1):
    ...
    if (param == None):
        ...
        part = db.execute('SELECT code FROM part WHERE title = \'{0}\''.format(value)).fetchone()
        if (part == None):
        ...
        locality = db.execute('SELECT code FROM locality WHERE title = \'{0}\''.format(value)).fetchone()
        if (locality == None):
            raise ValueError('Unknown locality {0}'.format(value))
        query = 'SELECT count(*) FROM street a, part b WHERE a.part = b.code AND b.locality = {0}'.format(locality[0])
        street_len = db.execute(query).fetchone()[0]
        query = """SELECT a.title as region, b.title as district, c.title as area, d.title as locality, e.title as part, e.zip, f.title as street
        ...
        area = db.execute('SELECT code FROM area WHERE title = \'{0}\''.format(value)).fetchone()
        if (area == None):
            raise ValueError('Unknown area {0}'.format(value))
        query = 'SELECT count(*) FROM street a, part b, locality c WHERE a.part = b.code AND b.locality = c.code AND c.area = {0}'.format(area[0])
        street_len = db.execute(query).fetchone()[0]
        query = """SELECT a.title as region, b.title as district, c.title as area, d.title as locality, e.title as part, e.zip, f.title as street
        ...
        district = db.execute('SELECT code FROM district WHERE title = \'{0}\''.format(value)).fetchone()
        if (district == None):
        ...
        region = db.execute('SELECT code FROM region WHERE title = \'{0}\''.format(value)).fetchone()
        if (region == None):
    ...
    for i in range(0,cnt):
        ...
        address['street_no'] = '{0}/{1}'.format(randint(100,10000), randint(1,99)) if (street_no_full) else str(randint(1,99))
        if (not dict_out):
            address = '{0} {1}, {2}, {3}'.format(address['street'], address['street_no'], address['part'], address['zip'])
        recs[i] = address
...
def create_type(title, description=None, col_titles=[]):
    ...
    try:        
        db = DBO(_get_dsn())._dbo_driver
        cnt = db.execute('SELECT count(*) FROM data_type WHERE TITLE = \'{0}\''.format(title)).fetchone()[0]
        if (cnt > 0):
        ...
        query += ', \'{0}\''.format(description) if (description != None) else ', null'
        col_len = len(col_titles)
        for i in range(0,10):
            query += ', \'{0}\''.format(col_titles[i]) if (i < col_len) else ', null'        
        query += ')'
        db.execute(query)
        db.commit()
...
def delete_type(title, del_records=True):
    ...
    try:
        ...
        if (del_records):
            query = 'DELETE FROM data WHERE type = (SELECT id FROM data_type WHERE title = \'{0}\')'.format(title)
            db.execute(query)    
...
def read_data(data_type, active=1, col_filter={}):
    ...
    try:
        db = DBO(_get_dsn())._dbo_driver  
        res = db.execute('SELECT id FROM data_type WHERE title = \'{0}\''.format(data_type)).fetchone()
        if (res == None):
...
def create_data(data_type, active=1, col_values={}):
    ...
    try:
        ...
        for i in range(1,11):
            query += '\'{0}\', '.format(col_values[i]) if (i in col_values.keys()) else 'null, '
        query = query[:-2] + ')' 
...
def delete_data(data_type, active=0, col_filter={}):
    ...
    try:
        ...
        query = 'DELETE FROM data WHERE type = {0} AND active = {1} AND '.format(type_id, active)
        for key, value in col_filter.items():
            query += 'col{0} = \'{1}\' AND '.format(key, value)
        query = query[:-5]
