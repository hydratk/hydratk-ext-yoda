...
+{bar}+""".format(bar=bar_len * '-', test_run_summary=test_run_summary)
        ...
        for test_set in test_sets:
            print("\n{0}".format(
                mh._trn.msg('yoda_test_set_summary', test_set['tset_id'].decode())))
                ...
                if ts['failures'] == True:
                    if ts['prereq_passed'] in (True, None):
                        if ts['prereq_passed'] == True:
                            print(
                                "    + {0}".format(mh._trn.msg('yoda_test_scenario_prereq_passed')))
                        ...
                        for tca in test_cases:
                            if tca['failures'] > 0:  # tca.failed_tco
                                print(
                                    "    {0}".format(mh._trn.msg('yoda_test_case', tca['value'].decode())))
                                ...
                                for tco in test_conditions:
                                    ...
                                    if tco['test_exec_passed'] == False:
                                        print(colorize("    ! {0}".format(mh._trn.msg(
                                            'yoda_test_condition_test_exec_failed', tco['log'].decode())), rgb=0xd70000))
                                    if tco['validate_exec_passed'] == False:
                                        print(colorize("    ! {0}".format(mh._trn.msg(
                                            'yoda_test_condition_validate_exec_failed', tco['log'].decode())), rgb=0xd70000))
