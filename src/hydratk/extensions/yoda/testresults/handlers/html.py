        ...
        if os.path.exists(src_file):
            if encoding == 'no':
                ...
                file_data = base64.b64encode(
                    file_get_contents(src_file, mode='rb')).decode()
            ...
            if content_type in (None, 'none'):
                ...
                charset_opt = "charset={0};".format(
                    charset) if charset is not None else ''
                res = "data:{content_type};{charset_opt}base64,{file_data}".format(
                    content_type=content_type, charset_opt=charset_opt, file_data=file_data)
...
class TestResultsOutputHandler(object):
    ...
    def create(self, test_run):
        ...
        if not os.path.exists(self._options['path']):
            print("Path {0} doesn't exists html report will be not created").format(
                self._options['path'])
            return False
        if not os.access(self._options['path'], os.W_OK):
            print("Path {0} is not writeable, permission denied").format(
                self._options['path'])
            return False
        ...
        test_run_data = self._db_con.db_data(
            "get_test_run", {'test_run_id': test_run.id})
        if (test_run_data == None or len(test_run_data) == 0):
            return False

        test_run_id = test_run_data[0]['name'] if test_run_data[
            0]['name'].decode() != 'Undefined' else test_run.id
        test_report_file = "{0}/{1}{2}.html".format(self._options['path'], datetime.datetime.fromtimestamp(
            int(test_run_data[0]['start_time'])).strftime('%Y-%m-%d_%H-%M-%S_'), test_run_id)
        ...
        if not os.path.exists(template_body_file):
            print("Missing template body.html {0}, report will be not created".format(
                template_body_file))
            return False
    ...
    def _format_custom_tco_opt(self, tco, tco_opt):
        ...
        """.format(
            ...
            start_time=datetime.datetime.fromtimestamp(
                int(tco['start_time'])).strftime('%Y-%m-%d %H:%M:%S'),
            end_time=datetime.datetime.fromtimestamp(
                int(tco['end_time'])).strftime('%Y-%m-%d %H:%M:%S'),
            total_time=round(tco['end_time'] - tco['start_time'], 3),
            test_resolution=tco['test_resolution'].decode() if hasattr(
                tco['test_resolution'], 'decode') else tco['test_resolution'],
            expected_result=tco['expected_result'].decode() if hasattr(
                tco['expected_result'], 'decode') else tco['expected_result'],
            test_result=tco['test_result'].decode() if hasattr(
                tco['test_result'], 'decode') else tco['test_result'],
        ...
        for opt_dict in tco_opt_rest:
            ...
            if opt_dict['opt_name'] is not None:
                tco_opt_rest_all[opt_dict['key']]['options'][
                    opt_dict['opt_name'].decode()] = opt_dict['opt_value'].decode()

        if len(tco_opt_rest_all) > 0:
            res += self._process_custom_data_opt(
                tco_opt_rest_all, 'TestCondition')
    ...
    def _format_custom_tc_opt(self, tc, tc_opt):
        ...
        """.format(
            tc_desc=tc_desc,
            start_time=datetime.datetime.fromtimestamp(
                int(tc['start_time'])).strftime('%Y-%m-%d %H:%M:%S'),
            end_time=datetime.datetime.fromtimestamp(
                int(tc['end_time'])).strftime('%Y-%m-%d %H:%M:%S'),
        ...
        for opt_dict in tc_opt_rest:
            ...
            if opt_dict['opt_name'] is not None:
                tc_opt_rest_all[opt_dict['key']]['options'][
                    opt_dict['opt_name']] = opt_dict['opt_value']
    ...
    def _format_custom_ts_opt(self, ts, ts_opt):
        ...
        """.format(
            ts_id=ts['ts_id'].decode() if hasattr(
                ts['ts_id'], 'decode') else ts['ts_id'],
            ts_name=ts['value'].decode() if hasattr(
                ts['value'], 'decode') else ts['value']
        )
        ...
        for opt_dict in ts_opt:
            if opt_dict['key'] == 'name':
                continue
            if opt_dict['key'] == 'desc':
                ts_desc = opt_dict['value'].decode() if hasattr(
                    opt_dict['value'], 'decode') else opt_dict['value']
                continue
            if opt_dict['key'] == 'path':
                ts_path = opt_dict['value'].decode() if hasattr(
                    opt_dict['value'], 'decode') else opt_dict['value']
                continue
            if opt_dict['key'] == 'author':
                ts_author = opt_dict['value'].decode() if hasattr(
                    opt_dict['value'], 'decode') else opt_dict['value']
                continue
            if opt_dict['key'] == 'version':
                ts_version = opt_dict['value'].decode() if hasattr(
                    opt_dict['value'], 'decode') else opt_dict['value']
                continue
        ...
        """.format(
            ...
            postreq=postreq,
            start_time=datetime.datetime.fromtimestamp(
                int(ts['start_time'])).strftime('%Y-%m-%d %H:%M:%S'),
            end_time=datetime.datetime.fromtimestamp(
                int(ts['end_time'])).strftime('%Y-%m-%d %H:%M:%S'),
        ...
        for opt_dict in ts_opt_rest:
            ...
            if opt_dict['opt_name'] is not None:
                ts_opt_rest_all[opt_dict['key']]['options'][
                    opt_dict['opt_name'].decode()] = opt_dict['opt_value'].decode()

        if len(ts_opt_rest_all) > 0:
            res += self._process_custom_data_opt(
                ts_opt_rest_all, 'TestScenario')
    ...
    def _process_custom_data_opt(self, opt_dict, td_class):
        ...
        for _, v in opt_dict.items():
            ...
            opt_name = v['key'] if 'label' not in v[
                'options'] else v['options']['label']
            ...
            res += """
               ...
               """.format(
                opt_name=opt_name.decode() if hasattr(
                    opt_name, 'decode') else opt_name,
                opt_value=v['value'].decode() if hasattr(
                    v['value'], 'decode') else v['value'],
                td_class=td_class
            )
        return res
    ...
    def get_test_results(self, test_run_id):
        ...
        test_sets = self._db_con.db_data(
            "get_test_sets", {'test_run_id': test_run_id})
...
=======
            test_set_end_time = "Not completed" if int(
                test_set['end_time']) == -1 else datetime.datetime.fromtimestamp(int(test_set['end_time'])).strftime('%Y-%m-%d %H:%M:%S')
...
=======
                test_set_id=test_set['tset_id'].decode() if hasattr(
                    test_set['tset_id'], 'decode') else test_set['tset_id'],
                test_set_start_time=datetime.datetime.fromtimestamp(
                    int(test_set['start_time'])).strftime('%Y-%m-%d %H:%M:%S'),
                test_set_end_time=test_set_end_time,
                test_set_total_time=0 if int(test_set[
                                             'end_time']) == -1 else round(test_set['end_time'] - test_set['start_time'], 3),
                test_set_total_tests=test_set['total_tests'],
                test_set_passed_tests=test_set['passed_tests'],
                test_set_failed_tests=test_set['failed_tests'],
                test_set_log=test_set['log'].decode().replace("\n", "<br>"),
            ...
            test_scenarios = self._db_con.db_data("get_test_scenarios", {
                                                  'test_run_id': test_run_id, 'test_set_id': test_set['id'].decode()})
>>>>>>> refs/remotes/origin/develop
...
=======
                ts_opt = self._db_con.db_data(
                    "get_test_custom_opt", {'test_object_id': ts['id']})
>>>>>>> refs/remotes/origin/develop
                res += """<tr>
...
=======
                ...
                test_cases = self._db_con.db_data("get_test_cases", {'test_run_id': test_run_id, 'test_set_id': test_set[
                                                  'id'].decode(), 'test_scenario_id': ts['id'].decode()})
                for tca in test_cases:
                    tca_opt = self._db_con.db_data(
                        "get_test_custom_opt", {'test_object_id': tca['id']})
>>>>>>> refs/remotes/origin/develop
                    res += """      
                    ...
                    test_conditions = self._db_con.db_data("get_test_conditions", {'test_run_id': test_run_id, 'test_set_id': test_set[
                                                           'id'].decode(), 'test_scenario_id': ts['id'].decode(), 'test_case_id': tca['id'].decode()})
                    for tco in test_conditions:
                        tco_opt = self._db_con.db_data(
                            "get_test_custom_opt", {'test_object_id': tco['id'].decode()})
                        res += """      
